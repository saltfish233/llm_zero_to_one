...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, pos_embedding):
        ...
        attn = self.attn_dropout(
            F.softmax(weights.float(), dim=-1).type_as(weights)
        )
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        freqs_cos, freqs_sin = precompute_freqs_cis(self.block_size, config.head_size, device=config.device)
        self.register_buffer(
...
class MyDataset(Dataset):
    def __init__(self, config,samples, tokenizer=None):
        self.block_size = config.block_size
...
if __name__ == "__main__":
    ...
    if local_rank == 0:
        full_ds = load_dataset("json", data_files=GPTConfig.data_path, split="train")
        split_data = full_ds.train_test_split(test_size=0.1, seed=42)
    else:
        split_data = None
    if dist.is_initialized():
        dist.barrier()
    ...
    train_ds = MyDataset(GPTConfig, split_data["train"], tokenizer)
    val_ds = MyDataset(GPTConfig, split_data["test"], tokenizer)
    ...
    for epoch in range(GPTConfig.num_epochs):
        ...
        if local_rank == 0:
            torch.save(checkpoint, f"checkpoints/20260514_epoch_{epoch}_valloss_{val_loss:.4f}")
